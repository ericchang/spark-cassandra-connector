...
class CassandraSQLSpec extends SparkCassandraITFlatSpecBase {
  ...
  it should "allow to select rows with >= clause" in {
    val result = cc.sql(s"SELECT * FROM test1 WHERE b >= 2").collect()
    result should have length 4
  }
  ...
  it should "allow to select rows with in clause" in {
    val result = cc.sql(s"SELECT * FROM test1 WHERE b in (1,2)").collect()
    result should have length 8
  }
  ...
  it should "allow to select rows with or clause" in {
    val result = cc.sql(s"SELECT * FROM test1 WHERE b = 2 or b = 1").collect()
    result should have length 8
  }
  ...
  it should "allow to select rows with not in clause" in {
    val result = cc.sql(s"SELECT * FROM test1 WHERE b not in (1,2)").collect()
    result should have length 0
  }

  it should "allow to select rows with is not null clause" in {
    val result = cc.sql(s"SELECT * FROM test1 WHERE b is not null").collect()
    result should have length 8
  }

  it should "allow to select rows with like clause" in {
    val result = cc.sql(s"SELECT * FROM test2 WHERE name LIKE '%om' ").collect()
    result should have length 1
  }

  it should "allow to select rows with between clause" in {
    val result = cc.sql(s"SELECT * FROM test2 WHERE a BETWEEN 1 AND 2 ").collect()
    result should have length 6
  }

  it should "allow to select rows with alias" in {
    val result = cc.sql(s"SELECT a AS a_column, b AS b_column FROM test2").collect()
    result should have length 9
  }

  it should "allow to select rows with distinct column" in {
    val result = cc.sql(s"SELECT DISTINCT a FROM test2").collect()
    result should have length 3
  }

  it should "allow to select rows with limit clause" in {
    val result = cc.sql(s"SELECT * FROM test1 limit 2").collect()
    result should have length 2
  }

  it should "allow to select rows with order by clause" in {
    val result = cc.sql(s"SELECT * FROM test1 order by d").collect()
    result should have length 8
  }
  ...
  it should "allow to select rows with union clause" in {
    val result = cc.sql(s"SELECT test1.a FROM $ks1.test1 AS test1 UNION DISTINCT SELECT test2.a FROM $ks1.test2 AS test2").collect()
    result should have length 3
  }
  ...
  it should "allow to select rows with union all clause" in {
    val result = cc.sql(s"SELECT test1.a FROM $ks1.test1 AS test1 UNION ALL SELECT test2.a FROM $ks1.test2 AS test2").collect()
    result should have length 17
  }

  it should "allow to select rows with having clause" in {
    val result = cc.sql(s"SELECT count(*) FROM test1 GROUP BY b HAVING count(b) > 4").collect()
    result should have length 0
  }

  it should "allow to select rows with partition column clause" in {
    val result = cc.sql(s"SELECT * FROM test1 WHERE a = 1 and b = 1 and c = 1").collect()
    result should have length 4
  }

  it should "allow to select rows with partition column and cluster column clause" in {
    val result = cc.sql(s"SELECT * FROM test1 WHERE a = 1 and b = 1 and c = 1 and d = 1 and e = 1").collect()
    result should have length 1
  }

  it should "allow to insert into another table" in {
    val result = cc.sql(s"INSERT INTO TABLE test3 SELECT a, b, c FROM test2").collect()
    val result2 = cc.sql(s"SELECT a, b, c FROM test3").collect()
    result2 should have length 9
  }

  it should "allow to insert into another table in different keyspace" in {
    val result = cc.sql(s"INSERT INTO TABLE $ks2.test3 SELECT test2.a, test2.b, test2.c FROM $ks1.test2 as test2").collect()
    val result2 = cc.sql(s"SELECT test3.a, test3.b, test3.c FROM $ks2.test3 as test3").collect()
    result2 should have length 9
  }

  it should "allow to join two tables" in {
    val result = cc.sql(s"SELECT test1.a, test1.b, test1.c, test2.a FROM $ks1.test1 AS test1 " +
      s"JOIN $ks1.test2 AS test2 ON test1.a = test2.a AND test1.b = test2.b AND test1.c = test2.c").collect()
    result should have length 4
  }

  it should "allow to join two tables from different keyspaces" in {
    val result = cc.sql(s"SELECT test1.a, test1.b, test1.c, test2.a FROM $ks1.test1 AS test1 " +
      s"JOIN $ks2.test2 AS test2 ON test1.a = test2.a AND test1.b = test2.b AND test1.c = test2.c").collect()
    result should have length 4
  }

  it should "allow to inner join two tables" in {
    val result = cc.sql(s"SELECT test1.a, test1.b, test1.c, test2.a FROM $ks1.test1 AS test1 " +
      s"INNER JOIN $ks1.test2 AS test2 ON test1.a = test2.a AND test1.b = test2.b AND test1.c = test2.c").collect()
    result should have length 4
  }

  it should "allow to left join two tables" in {
    val result = cc.sql(s"SELECT test1.a, test1.b, test1.c, test1.d, test1.e, test1.f FROM $ks1.test1 AS test1 " +
      s"LEFT JOIN $ks1.test2 AS test2 ON test1.a = test2.a AND test1.b = test2.b AND test1.c = test2.c").collect()
    result should have length 8
  }

  it should "allow to left outer join two tables" in {
    val result = cc.sql(s"SELECT test1.a, test1.b, test1.c, test1.d, test1.e, test1.f FROM $ks1.test1 AS test1 " +
      s"LEFT OUTER JOIN $ks1.test2 AS test2 ON test1.a = test2.a AND test1.b = test2.b AND test1.c = test2.c").collect()
    result should have length 8
  }

  it should "allow to right join two tables" in {
    val result = cc.sql(s"SELECT test2.a, test2.b, test2.c FROM $ks1.test1 AS test1 " +
      s"RIGHT JOIN $ks1.test2 AS test2 ON test1.a = test2.a AND test1.b = test2.b AND test1.c = test2.c").collect()
    result should have length 12
  }

  it should "allow to right outer join two tables" in {
    val result = cc.sql(s"SELECT test2.a, test2.b, test2.c FROM $ks1.test1 AS test1 " +
      s"RIGHT OUTER JOIN $ks1.test2 AS test2 ON test1.a = test2.a AND test1.b = test2.b AND test1.c = test2.c").collect()
    result should have length 12
  }

  it should "allow to full join two tables" in {
    val result = cc.sql(s"SELECT test2.a, test2.b, test2.c FROM $ks1.test1 AS test1 " +
      s"FULL JOIN $ks1.test2 AS test2 ON test1.a = test2.a AND test1.b = test2.b AND test1.c = test2.c").collect()
    result should have length 16
  }

  it should "allow to select rows for collection columns" in {
    val result = cc.sql(s"SELECT * FROM test_collection").collect()
    result should have length 1
  }
  ...
  it should "allow to select specified non-UDT columns from a table containing some UDT columns" in {
    val result = cc.sql(s"SELECT key, name FROM udts").collect()
    result should have length 1
    val row = result.head
