...
object DynamicCassandraPorts extends Logging {
  ...
  private val basePorts = 10000 to 20000 by 50
  ...
  def verifyRange(host: String, ports: Seq[Int]): Boolean = {
  ...
  def chooseBasePort(): Int = {
    ...
    val foundPort = basePorts.find { basePort =>
      ...
      Try(Files.createFile(file)) match {
        case Success(filePath) =>
          ...
          if (verifyRange("127.0.0.1", basePort until (basePort + 100))) {
            true
          } else {
    ...
    if (foundPort.isDefined) foundPort.get
      ...
      println("All ports are busy. Waiting...")
      Thread.sleep(1000)
      chooseBasePort()
    }
  }
